...
async function failure(upstream: Response, cookies: string[], fallback: string): Promise<Response> {
  ...
  if (upstream.status >= 500) {
    return json({ error: fallback, code: 'PROMOTION_UPSTREAM_UNAVAILABLE' }, 502, cookies);
  }
  return json({ error: message, code: pgDetail || 'PROMOTION_VALIDATION_FAILED' }, 400, cookies);
}

export async function handleAdminPromotions(
  request: Request,
  ...
  const upstream = await rpc('get_promotion_admin_state', {}, auth.accessToken, deps);
  if (!upstream.ok) return failure(upstream, auth.responseCookies, 'Promotion configuration is unavailable');
  const data = await upstream.json().catch(() => null);
  if (!Array.isArray(data)) {
    return json({ error: 'Promotion response is invalid', code: 'PROMOTION_RESPONSE_INVALID' }, 502, auth.responseCookies);
  }
  return json({ data }, 200, auth.responseCookies);
}

export async function handleAdminSavePromotion(
  request: Request,
  deps: EmployeeBffDependencies = {},
): Promise<Response> {
  ...
  const upstream = await rpc('save_promotion', { p_payload: body }, auth.accessToken, deps);
  if (!upstream.ok) return failure(upstream, auth.responseCookies, 'Promotion update failed');
  const data = await upstream.json().catch(() => null);
  if (!Array.isArray(data) || data.length !== 1) {
    return json({ error: 'Promotion update response is invalid', code: 'PROMOTION_RESPONSE_INVALID' }, 502, auth.responseCookies);
  }
  return json({ data: data[0] }, 200, auth.responseCookies);
}
